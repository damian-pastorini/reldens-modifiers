...
class Modifier
{
    ...
    constructor(props)
    {
        ...
        this.operation = Number(props.operation);
        ...
        this.minValue = sc.get(props, 'minValue', false);
        this.maxValue = sc.get(props, 'maxValue', false);
    ...
    validateConditions(target)
    {
        for(let condition of this.conditions){
            if(!(condition instanceof Condition)){
                Logger.error('Missing Condition instance.', (typeof condition));
    ...
    getModifiedValue(revert = false, useBasePropertyToGetValue = false)
    {
        ...
        // get the current property value (this could be already modified):
    ...
    applyModifierLimits(ownerProperty)
    {
        ...
        if('number' === typeof this.minValue && ownerProperty < this.minValue){
            ownerProperty = this.minValue;
        }
        if('number' === typeof this.maxValue && ownerProperty > this.maxValue){
